...
pub struct Journal {
    /// All transactions in the journal along with their hash values.
    pub transactions: Vec<(u64, transaction::Transaction)>,
    /// All price directives in the journal along with their hash values.
    pub prices: Vec<(u64, price::PriceDirective)>,
}
...
impl Journal {
    ...
    /// ```
    pub fn new(
        transactions: Vec<(u64, transaction::Transaction)>,
        prices: Vec<(u64, price::PriceDirective)>,
    ) -> Self {
        Self {
            transactions,
            prices,
...
fn read_journal_file(
    ...
    Vec<(u64, transaction::Transaction)>,
    Vec<(u64, price::PriceDirective)>,
)> {
    // Initialise empty vectors for transactions and prices
    let mut transactions: Vec<(u64, transaction::Transaction)> = Vec::new();
    let mut prices: Vec<(u64, price::PriceDirective)> = Vec::new();
    ...
    loop {
        ...
        match directive_type {
            ...
            utils::DirectiveType::Transaction => {
                ...
                transactions.push((transaction_hash, transaction));
            }
            utils::DirectiveType::Price => {
                ...
                let price_hash: u64 = hasher.finish();
                prices.push((price_hash, price));
            }
